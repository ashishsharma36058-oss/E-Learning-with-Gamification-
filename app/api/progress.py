...
async def stats(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for lang in ["python", "javascript", "cpp", "java"]:
        total_in_lang = db.query(Challenge).filter(Challenge.language == lang).count()
        completed_in_lang = db.query(UserProgress).join(Challenge).filter(
        ...
        lang_stats[lang] = {
            "total": total_in_lang,
            "completed": completed_in_lang,
            "pct": round(completed_in_lang / max(total_in_lang, 1) * 100, 1)
        }
