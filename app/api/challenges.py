...
def run_tests(code: str, test_cases: list, language: str):
    ...
    return False, f"{passed}/{total} tests passed"
...
async def list_challenges(
    ...
    # Show ALL challenges — no level gating on list
    q = db.query(Challenge)
    if language:
        q = q.filter(Challenge.language == language)
    if difficulty:
...
async def submit(
    ...
    streak_bonus = False
    if body.time_taken < ch.time_limit * 0.5:
        xp = int(xp * 1.3)
    if current_user.streak_days >= 5:
        xp = int(xp * 1.5)
        streak_bonus = True

    prog.completed = True
    prog.xp_earned = xp
    prog.time_taken = body.time_taken
    prog.hints_used = body.hints_used
